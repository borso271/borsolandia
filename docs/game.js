...
function resizeCanvas() {
    ...
    const gameWidth = Math.min(viewportWidth * 0.9, viewportHeight * 0.9);
    canvas.width = gameWidth;
    canvas.height = gameWidth; // Keep it square for the maze

    // Adjust cell size based on the new canvas width
    CELL_SIZE = Math.floor(canvas.width / COLS); // COLS is the number of columns in the maze
    inset = 0; //CELL_SIZE/100;
    borderWidth = CELL_SIZE/30;
