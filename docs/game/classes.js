...
export class ImmovableWall {
    ...
    export class Wall {
        ...
        render(ctx) {
            const innerBorderWidth = 0.5; // Thin inner border width
            const mainBorderWidth = borderWidth; // Main border width for walls
            ...
            this.coordinates.forEach(([y, x]) => {
                ...
                if (this.isPlayer) {
                    // Render the player as a circle with glow effect
                    ...
                    // Thin inner borders where there are no neighboring walls
                    ...
                    // Main wall borders, drawn within the cell dimensions
                    ctx.lineWidth = mainBorderWidth;
                    ctx.strokeStyle = this.borderColor;
        
                    ctx.beginPath();
                    if (!top) {
                        ctx.moveTo(xPos + mainBorderWidth / 2, yPos + mainBorderWidth / 2);
                        ctx.lineTo(xPos + CELL_SIZE - mainBorderWidth / 2, yPos + mainBorderWidth / 2);
                    }
                    if (!right) {
                        ctx.moveTo(xPos + CELL_SIZE - mainBorderWidth / 2, yPos + mainBorderWidth / 2);
                        ctx.lineTo(xPos + CELL_SIZE - mainBorderWidth / 2, yPos + CELL_SIZE - mainBorderWidth / 2);
                    }
                    if (!bottom) {
                        ctx.moveTo(xPos + CELL_SIZE - mainBorderWidth / 2, yPos + CELL_SIZE - mainBorderWidth / 2);
                        ctx.lineTo(xPos + mainBorderWidth / 2, yPos + CELL_SIZE - mainBorderWidth / 2);
                    }
                    if (!left) {
                        ctx.moveTo(xPos + mainBorderWidth / 2, yPos + CELL_SIZE - mainBorderWidth / 2);
                        ctx.lineTo(xPos + mainBorderWidth / 2, yPos + mainBorderWidth / 2);
                    }
                    ctx.stroke();

                    // Example: Plus symbol in the center
