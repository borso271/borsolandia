...
function displayLevelAndMoves() {
    ...
    ctx.fillStyle = "#00ff00"; // Neon green for level text
    ...
    moveDigits.forEach((digit, index) => {
        ...
        ctx.fillStyle = "#ff3300"; // Bright red for moves text
...
for (let y = 0; y < ROWS; y++) {
    ...
    if (exitPosition) {
        ...
        const yPos = exitY * CELL_SIZE;
          ...
          ctx.fillStyle = "#ff00ff"; // Bright violet-pink for the diamond
          ctx.font = `${CELL_SIZE * 0.9}px Arial`; // Font size relative to cell size
          ctx.textAlign = "center";
          ctx.textBaseline = "middle";
      
          // Draw glow effect by creating a shadow
          ctx.shadowColor = "#ff66ff"; // Lighter pink for the glow
